...
const Timeline = () => {
  ...
  const handleClick = (e, i) => {
    ...
    if (carouselRef.current) {
      const scrollLeft = Math.floor(carouselRef.current.scrollWidth * 0.7 * (i / activeItem.length));
  ...
  // const handleScroll = () => {
  //   if (carouselRef.current) {
  //     const index = Math.round((carouselRef.current.scrollLeft / (carouselRef.current.scrollWidth * 0.7)) * activeItem.length);
        ...
        <>
          {activeItem.map((item, index) => (
            <CarouselMobileScrollNode
              key={index}
              final={index === activeItem.length - 1}>
              <CarouselItem
                index={index}
